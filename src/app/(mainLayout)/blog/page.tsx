...
import { HeaderSection, BlogSection, FaqSection } from './_components';
...
export async function generateMetadata({ searchParams }: GenerateMetadataParams) {
  ...
  return {
    ...
    alternates: {
      ...
      prev: page > 1 ? `/blog?category=${category}&page=${page - 1}` : null,
      next: page < totalPages ? `/blog?category=${category}&page=${page + 1}` : null,
    },
    openGraph: {
      url,
      images: [
...
export default async function Blog({ searchParams }: { searchParams: SearchParams }) {
  ...
  const tags = await tagsApi.getTags(true);
  const featuredPost = await postsApi.getFeaturedPostShortInfo(true);
  const populateFaqs = await faqApi.getPopulateFaqsByPage('home');

  return (
    <div>
      <HeaderSection />
      <BlogSection isBlog posts={currentPagePosts} totalPages={totalPages} tags={tags} featuredPost={featuredPost} />
